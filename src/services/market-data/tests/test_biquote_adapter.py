...
from tests.conftest import BIQUOTE_OHLC, make_client
...
async def test_get_close_returns_validated_observation() -> None:
    adapter = _adapter_returning(BIQUOTE_OHLC)
    obs = await adapter.get_close(AssetId.XOM_NYSE, date(2026, 7, 13))
    assert obs.session == date(2026, 7, 13)
    assert obs.close == Decimal("3290.25")
    assert obs.provider_symbol == "XOM"
...
async def test_missing_session_raises_not_yet_available() -> None:
    ...
    with pytest.raises(PriceNotYetAvailableError):
        await adapter.get_close(AssetId.XOM_NYSE, date(2026, 7, 14))
...
async def test_non_positive_and_null_closes_are_skipped() -> None:
    payload = copy.deepcopy(BIQUOTE_OHLC)
    payload["bars"] = [
    ...
    adapter = _adapter_returning(payload)
    observations = await adapter.fetch_observations(AssetId.XOM_NYSE, date(2026, 7, 13))
...
async def test_missing_bars_block_is_terminal() -> None:
    adapter = _adapter_returning({"symbol": "XOM", "interval": "1d"})
    with pytest.raises(InvalidObservationError):
        await adapter.get_close(AssetId.XOM_NYSE, date(2026, 7, 13))

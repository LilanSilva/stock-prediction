...
> **POC-7 note (2026-07-30):** Moot. With the provider migrated to biquote.io ([POC-7](../../POC/poc-7-biquote-price-source.md)), the primary is `BiquoteAdapter` and there is still **no fallback adapter** (`fallback: null` in `biquote-reference-v1`). Stooq was never viable (see the 2026-07-28 spike below) and is not revisited. This task remains not-implemented.

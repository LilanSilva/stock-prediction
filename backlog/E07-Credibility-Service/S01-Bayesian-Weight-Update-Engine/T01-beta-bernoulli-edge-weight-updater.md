...
> The original checklist below predates the contract freeze (`scored-predictions` queue, arbitrary
> `edge_id` Neo4j property, per-service `requirements.txt`, aio-pika manual ack). It is retained for
> history; superseded lines are marked and the authoritative outcome is the **As-built** checklist
> that follows.

- [x] Unit tests pass (`pytest src/services/credibility/tests/`)
- [x] `ruff check src/services/credibility/` exits 0
- [x] `mypy src/services/credibility/` exits 0 *(run as `mypy --strict`, matching the sibling services)*
- [x] `compute_proportional_credits` is tested with: normal case, single edge, zero-sum weights, empty list
- [~] Neo4j update is tested with a mock driver asserting the correct Cypher parameters — *superseded: the Cypher lives in the shared `CausalGraphClient`; the pipeline is tested against a `FakeGraph` asserting the correct `update_edge_weight(factor_id, asset_id, alpha, beta)` calls*
- [~] Postgres upsert is tested with an in-memory or mock session — *superseded: source upsert + history + idempotency are covered by a live-Postgres integration test (`test_integration.py`) and a `FakeRepo` unit test*
- [~] Message ack/nack behaviour is tested for success, Neo4j failure, and malformed message cases — *superseded: ack/nack is owned by the shared `RabbitMQClient`; the service maps a malformed `edge_id` to `MessagePoisonError` (dead-letter) and a duplicate to a no-op, both unit-tested*
- [~] `src/services/credibility/requirements.txt` lists all dependencies with minimum version pins — *superseded: deps are declared in `pyproject.toml`; the repo-root hashed `requirements.txt` is regenerated by `pip-compile` across all services and copied in by the Dockerfile*
- [x] `src/services/credibility/Dockerfile` exists and builds successfully

### As-built (implemented 2026-07-29)

- [x] `credibility/updater.py` `compute_proportional_credits` normalises credit by total `influence_weight` (equal-split fallback on zero sum) and `apply_bernoulli` adds credit to `alpha` (hit) or `beta` (miss) with a prior floor
- [x] `credibility/pipeline.py` parses each `edge_id` (`"FACTOR->ASSET"`) to its `(EventType, AssetId)` keys, reads current `alpha`/`beta` via the shared `get_firing_edges`, and writes the new counts back via `update_edge_weight`; a missing edge logs `edge_missing_in_graph` and is skipped
- [x] Verified end-to-end against the live stack: a real hit on `SANCTIONS->BRENT_OIL` moved the Neo4j edge from `alpha=1.0` to `alpha=2.0` (beta unchanged)
- [x] `pyproject.toml` pins `asyncpg`, `apscheduler`, `uvicorn`, `neo4j`; `Dockerfile` builds and `feed-credibility` runs healthy
